...
const currencyDataPack = JSON.parse(`
    [
...
var slrisk = document.getElementById("slrisk-input");
var tprisk = document.getElementById("tprisk-input");
...
slrisk.addEventListener("input", async function(event) {
    if(calculateButton.style.display === "none"){
        calculateResult();
    }
});

tprisk.addEventListener("input", async function(event) {
    if(calculateButton.style.display === "none"){
        calculateResult();
    }
});
